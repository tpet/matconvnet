function y = vl_nnreshape(x, dims, dzdy)
...
%
% https://github.com/vlfeat/matconvnet/issues/729#issuecomment-253926725
%

assert(sum(dims == -1) <= 1, ...
    'At most one dim can be computed from the others.');
assert(length(dims) == 3, ...
    'Dims should have three elements');
